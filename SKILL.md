...
1. 需求命中策略、任务等完整业务主题时，先读取 `references/01-theme/` 下对应主题文档。
2. 页面包含导入、导出、标签、优先级、执行周期、批量编辑等局部能力时，读取 `references/02-feature/` 下对应功能点文档。
3. 需要判断页面类型、导航层级、表格、表单、交互、状态或术语时，读取 `references/03-template/` 与 `references/04-patterns/` 下对应基础设计文档；判断页面类型时，先读 `references/04-patterns/01-navigation-and-hierarchy.md` 走页面类型选择决策链确定页面类型，再读 `references/03-template/01-page-types.md` 匹配具体模板和线框。
...
- 示例1：
  ...
  - 读取资源：先读 `references/01-theme/strategy-management.md`，再按需读取表格、表单和交互规范。
...
- 主题设计库：见 [references/01-theme/strategy-management.md](references/01-theme/strategy-management.md)（何时读取：需求涉及策略、规则、白名单、规则组、策略优先级或策略生命周期时）
